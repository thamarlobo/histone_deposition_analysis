...
# Script the outputfile .*deltarfs.txt from detect_strandswitches.pl as input.
...
# Outputformat: chromosome   start(=origin start)   stop(=origin stop)  ratio(=delta_fr ratio)  deltafr leftf(=number of fragments from forward strand to left of origin)   rightr(=number of fragments from reverse strand to right of origin). Columns are tab-separated.
#
